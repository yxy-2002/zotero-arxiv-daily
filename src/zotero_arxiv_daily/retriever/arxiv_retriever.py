...
ARXIV_INITIAL_DELAY_SECONDS = 30
ARXIV_RETRY_BACKOFF_SECONDS = [60, 180, 600]
...
class ArxivRetriever(BaseRetriever):
    ...
    def _retrieve_raw_papers(self) -> list[ArxivResult]:
        ...
        if all_paper_ids:
            logger.info(
                f"Retrieved {len(all_paper_ids)} arXiv IDs from RSS. "
                f"Waiting {ARXIV_INITIAL_DELAY_SECONDS}s before API requests."
            )
        ...
        for i in range(0, len(all_paper_ids), ARXIV_BATCH_SIZE):
            ...
            for attempt, delay in enumerate([0] + ARXIV_RETRY_BACKOFF_SECONDS):
                if delay:
                    logger.warning(
                        f"arXiv API may be rate limited. "
                        f"Retrying batch {i // ARXIV_BATCH_SIZE + 1} in {delay}s..."
                    )
                    time.sleep(delay)
    ...
    with TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            if "all" not in file_contents:
                logger.warning(
                    f"Failed to extract full text of {paper.title} from tar: "
